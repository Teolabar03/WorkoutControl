...
export function useRestTimer(sessioneId: number) {
  ...
  const avvia = useCallback(
    (secondi: number) => {
      ...
      localStorage.setItem(chiave, String(Date.now() + secondi * 1000))
      setVisibile(true)
      setFinito(false)
      attivaWakeLock()
    ...
    [chiave, preparaAudio, attivaWakeLock, disegnaTimer]
  )
  ...
  const piuTrenta = useCallback(() => {
    ...
    localStorage.setItem(chiave, String(base + 30_000))
    suonatoRef.current = false
    setFinito(false)
    disegnaTimer()
  }, [chiave, fineSalvata, disegnaTimer])
